...
public class FoxFlowerRender<T extends EntityAbstractSummonedSword> extends EntityRenderer<T> {
    ...
    public FoxFlowerRender(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        try (MSAutoCloser ignored = MSAutoCloser.pushMatrix(matrixStack)) {
            ...
            BladeRenderState.renderOverridedLuminous(ItemStack.EMPTY, model, "lmhr", TEXTURE, matrixStack, bufferIn,
                    packedLightIn);
        }
    }
}
