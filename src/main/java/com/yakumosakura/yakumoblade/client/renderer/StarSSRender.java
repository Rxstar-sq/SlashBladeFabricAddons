...
public class StarSSRender<T extends EntityAbstractSummonedSword> extends EntityRenderer<T> {
    ...
    public StarSSRender(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        try (MSAutoCloser ignored = MSAutoCloser.pushMatrix(matrixStack)) {
            ...
            BladeRenderState.renderOverridedLuminous(ItemStack.EMPTY, model, "pao", TEXTURE, matrixStack, bufferIn,
                    packedLightIn);
        }
    }
}
