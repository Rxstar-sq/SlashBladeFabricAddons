...
public class FoxFlowerSuperRender<T extends EntityAbstractSummonedSword> extends EntityRenderer<T> {
    private static final ResourceLocation TEXTURE = Yakumoblade.prefix("model/named/util/ss.png");
    private static final ResourceLocation MODEL = Yakumoblade.prefix("model/named/util/lmhr.obj");
    ...
    public FoxFlowerSuperRender(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        try (MSAutoCloser ignored = MSAutoCloser.pushMatrix(matrixStack)) {
            ...
            BladeRenderState.renderOverridedLuminous(ItemStack.EMPTY, model, "lhyj", TEXTURE, matrixStack, bufferIn,
                    packedLightIn);
        }
    }
}
