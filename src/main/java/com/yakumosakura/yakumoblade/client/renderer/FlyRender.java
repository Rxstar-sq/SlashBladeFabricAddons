...
public class FlyRender<T extends EntityAbstractSummonedSword> extends EntityRenderer<T> {
    private static final ResourceLocation TEXTURE = Yakumoblade.prefix("model/named/util/ss.png");
    private static final ResourceLocation MODEL = Yakumoblade.prefix("model/named/util/butterfly.obj");
    ...
    public FlyRender(EntityRendererProvider.Context context) {
        super(context);
    }
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack matrixStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        try (MSAutoCloser ignored = MSAutoCloser.pushMatrix(matrixStack)) {
            ...
            BladeRenderState.renderOverridedLuminous(ItemStack.EMPTY, model, "wings", TEXTURE, matrixStack, bufferIn,
                    packedLightIn);
        }
    }
}
