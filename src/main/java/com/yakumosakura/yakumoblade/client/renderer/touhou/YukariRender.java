package com.yakumosakura.yakumoblade.client.renderer.touhou;
...
public class YukariRender<T extends YukariEntity> extends EntityRenderer<T> {
    public YukariRender(EntityRendererProvider.Context context) {
        super(context);
    }

    private static final ResourceLocation TEXTURE = Yakumoblade.prefix("model/util/touhou/dead_or_life.png");
    private static final ResourceLocation MODEL = Yakumoblade.prefix("model/util/touhou/dead_or_life.obj");
    ...
    public void render(T entity, float entityYaw, float partialTicks, PoseStack matrixStack,
        ...
        try (MSAutoCloser msac = MSAutoCloser.pushMatrix(matrixStack)) {
            ...
            try (MSAutoCloser clockwiseMsac = MSAutoCloser.pushMatrix(matrixStack)) {
                matrixStack.mulPose(Axis.YP.rotationDegrees(rotationAngle));
                BladeRenderState.renderOverrided(ItemStack.EMPTY, model, "dead_or_life", TEXTURE, matrixStack, buffer, packedLight);
                BladeRenderState.renderOverridedLuminous(ItemStack.EMPTY, model, "dead_or_life", TEXTURE, matrixStack, buffer, packedLight);
            }
